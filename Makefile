...
firmware-pubkey-header:
	...
	@_hex=$$(openssl pkey -in $(FW_SIGN_KEY) -pubout -outform DER | tail -c 32 | od -An -tx1 -v | tr -d ' \n'); \
...
firmware: rootfs
	...
	@# Ed25519-sign the FIT image (detached 64-byte signature). The device
	@# verifies this with the compiled-in public key before flashing.
	openssl pkeyutl -sign -inkey $(FW_SIGN_KEY) -rawin \
		-in $(BUILD_DIR)/firmware/stargazer.itb \
